...
export class ClipperController {
  ...
  async postClipper() {
    // m3u8 주소 획득
    const m3u8 = await this.clipperService.getM3u8('arahashitabi_stellive');
    ...
    const parser = await this.clipperService.getParser(m3u8_data);
  ...
  async getM3u8(@Query('user_id') user_id: string) {
    return await this.clipperService.getM3u8(user_id);
  }
}
